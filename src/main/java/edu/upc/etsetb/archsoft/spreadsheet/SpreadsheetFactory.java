...
/**
 * Factory class of the project. It Creates all the different elements in this
 * project
 *
...
public class SpreadsheetFactory {
    ...
    /**
     * Returns a Function of the given type. The type is defined by the String
     * entered by the user. If the type does not exist, it throws the unknown
     * type exception
     *
    ...
    /**
     * Function that returns a FormulaElement object containing the given
     * information.
     *
     * @param token numeric identifier corresponding to each type of possible
     * token.
     * @param tok sequence from original input
     * @param tokens list containing all tokens. Used by CellRange to break down
     * into small CellReferences
     * @return an object (or objects) of type corresponding to given "token"
     * with "tok" sequence content
    ...
    /**
     * Here we break down a CellRange sequence into it's top-left cell reference
     * and it's bottom-right cell reference. All the cells inbetween are then
     * added into the "tokens" linked list in an iterating loop across the
     * dimensions of the CellRange lastly the final cell reference is returned
     * and added to the list on the function that called this method
     *
     * @param sequence sequence containing top-left cell : bottom-right cell for
     * a cell range
     * @param tokens linked list of tokens where we add all cells within cell
     * range extracted
     * @return
     */
